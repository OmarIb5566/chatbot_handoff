"""Round 3 Part C: unambiguous Q1 then pronoun follow-up."""
import json, sys
...
out = []
for label, q in (("Q1", "Who prepares the subcontract amendment request?"),
                 ("Q2", "who signs after that?")):
    g = looks_like_followup(q, has_history=(label == "Q2"))
    print(f"\n--- {label}: {q}")
    print(f"  gate: is_followup={g.is_followup}  reasons={list(getattr(g,'reasons',[]) or [])}")
    rec = bot.ask(q, session_id="R3")
    d = {"label": label, "question": q, "gate_is_followup": g.is_followup,
         **{k: rec.get(k) for k in ("workflow_intent","n_workflow_hits","route",
         ...
         "hits": [{"score": round(float(h["score"]),4), "filename": h["filename"],
                  ...
                  for h in rec["hits"]],
         "answer": (rec.get("display") or "")[:2400]}
    out.append(d)
    print(f"  rec gate_reasons: {d['gate_reasons']}")
    print(f"  was_rewritten={d['was_rewritten']} reused_sources={d['reused_sources']} "
          f"rewrite_model={d['rewrite_model']}")
    print(f"  REWRITTEN QUERY: {d['query_used']!r}")
    print(f"  workflow_intent={d['workflow_intent']} n_wf={d['n_workflow_hits']}/{len(d['hits'])}")
    for h in d["hits"]:
        print(f"    [{h['score']:.4f}] {str(h['source_type']):15s} {h['filename'][:60]:60s} | {h['section']}")
    print("  ANSWER:\n    " + d["answer"][:1400].replace("\n", "\n    "))
    (ROOT/"_verify"/"results"/"15_r3_followup.json").write_text(
