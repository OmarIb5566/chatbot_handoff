...
- **RME's own department and role vocabulary is masked the same way** (`glossary.py`,
  `mask_glossary`/`unmask_glossary`). Generic MT does not know that "Commercial" is this corpus's
  department name rather than a synonym for "trade" — `قسم التجارة` used to come back as "trade
  department", dropping a word that is both a BM25 term and a routing keyword (`PTN01-3`).
  Unambiguous terms (`PM`, `HoD`, `CM`, `Formwork`, `VMO`, …) are pinned to their exact corpus
  spelling before the model ever sees the Arabic; abbreviations the corpus itself uses for more
  than one role (`TM`, `MS`, `IIR`) are left alone rather than guessed at. The same table runs in
  reverse for `to_arabic()`, which is what keeps `make_arabic_eval.py` from mistranslating its own
  eval questions (see below).
...
**Measured:** retrieval hit rate on the Arabic path, via `evals/eval_arabic.py` — the one metric
whose ground truth is language-independent, so it sits on the same scale as the English number.

```bash
python evals/make_arabic_eval.py   # eval_set.json -> eval_set_ar.json (needs Ollama)
python evals/eval_arabic.py        # English vs Arabic->English vs raw Arabic, top-3 doc accuracy
```

```
English (baseline)    27/32 =  84.4%
Arabic -> English     27/32 =  84.4%      <- now matches English exactly
Arabic (raw, no MT)    9/32 =  28.1%      <- control
```
...
Before the glossary fix, the Arabic row was one miss behind English: PTN01-3 asks about the
*Commercial dept*, the Arabic renders it `قسم التجارة`, and that came back as "trade department".
A translation failure, not a retrieval one, and the silent kind. `glossary.py` closes it by
pinning RME's vocabulary rather than trusting the prompt to — the remaining misses on both rows
(`PTN02-3`, `PVMO01-1`, `PVMO01-3`, `PVMO02-3`, `PVMO02-4`) are pre-existing retrieval gaps that
also miss in English, not translation defects.
